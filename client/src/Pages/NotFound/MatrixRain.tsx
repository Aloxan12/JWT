...
// Constants
const VALID_CHARS = `404 page not found`;
...
const getMutatedStream = (stream: any) => {
    const newStream = [];
...
const RainStream = (props: any) => {
    ...
    const [intervalDelay, setIntervalDelay] = useState<null | any>(null);
...
export const MatrixRain = (props: any) => {
    const containerRef = useRef<HTMLDivElement | null>(null);
    const [containerSize, setContainerSize] = useState<null | {width?: number, height?: number}>(null); // ?{width, height}
    ...
    const streamCount = containerSize ? Math.floor(containerSize?.width || 1 / 26) : 0;
